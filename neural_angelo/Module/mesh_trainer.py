...
class MeshTrainer(Trainer):
    ...
    def _get_scene_aabb_from_config(self) -> torch.Tensor:
        ...
        """
        # 优先使用 radius (与 model.py 一致)，确保 OccupancyGrid 覆盖整个场景
        if hasattr(self.cfg.model, 'radius'):
            radius = self.cfg.model.radius
            aabb = torch.tensor([-radius, -radius, -radius, radius, radius, radius], 
                                dtype=torch.float32, device=self.device)
        else:
            # 回退到 hashgrid.range
            hashgrid_range = self.cfg.model.object.sdf.encoding.hashgrid.range
            aabb = torch.tensor([
                hashgrid_range[0], hashgrid_range[0], hashgrid_range[0],  # min
                hashgrid_range[1], hashgrid_range[1], hashgrid_range[1]   # max
            ], dtype=torch.float32, device=self.device)
    ...
    def _setup_frozen_occupancy_grid_from_mesh(self) -> bool:
        ...
        print(f'\t Config hashgrid.range: {self.cfg.model.object.sdf.encoding.hashgrid.range}')
        print(f'\t Scene AABB (aligned): {scene_aabb.tolist()}')
        print(f'\t Grid resolution: {resolution}')

        # 检查 AABB 是否大于 hashgrid.range，如果是，打印警告
        hashgrid_range = self.cfg.model.object.sdf.encoding.hashgrid.range
        if scene_aabb[3] > hashgrid_range[1] or scene_aabb[0] < hashgrid_range[0]:
            print(f'[WARN] Scene AABB is larger than hashgrid.range {hashgrid_range}.')
            print(f'       Points outside hashgrid.range might be normalized to > 1.0 or < 0.0.')
            print(f'       Consider updating hashgrid.range in config to match your mesh/radius.')
        ...
        print('\t OccupancyGrid has been frozen and aligned with hashgrid.range.')
    ...
    def _compute_mesh_occupancy_grid(
        ...
        # 形态学膨胀：确保边缘体素被正确包含
        # 由于体素中心到 AABB 边界有 0.5*voxel_size 的偏移，需要膨胀来补偿
        binary_grid_3d = binary_grid.view(resolution, resolution, resolution)
        binary_grid_3d = self._dilate_occupancy_grid(binary_grid_3d, iterations=1)
        binary_grid = binary_grid_3d.view(-1)
        
        print(f'[DEBUG] After dilation, occupied voxels: {binary_grid.sum().item()} / {binary_grid.numel()}')

        return binary_grid.to(device)
    
    def _dilate_occupancy_grid(self, grid_3d: torch.Tensor, iterations: int = 1) -> torch.Tensor:
        """对 3D occupancy grid 进行形态学膨胀。

        使用 3x3x3 的结构元素进行膨胀，确保边缘体素被正确包含。
        这可以补偿体素中心与 AABB 边界之间的偏移。

        Args:
            grid_3d: [D, H, W] 的 bool tensor。
            iterations: 膨胀迭代次数。

        Returns:
            dilated_grid: [D, H, W] 的 bool tensor。
        """
        import torch.nn.functional as F
        
        # 转换为 float 并添加 batch 和 channel 维度
        grid_float = grid_3d.float().unsqueeze(0).unsqueeze(0)  # [1, 1, D, H, W]
        
        # 使用 3x3x3 的 max pooling 进行膨胀
        for _ in range(iterations):
            grid_float = F.max_pool3d(grid_float, kernel_size=3, stride=1, padding=1)
        
        # 转换回 bool
        dilated_grid = (grid_float > 0.5).squeeze(0).squeeze(0)
